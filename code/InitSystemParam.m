...
param.batchSize = 50; % we found batch size of 10 is faster and gives the same results

param.cropHeight = param.height - 2 * param.cropSizeTraining;
param.cropWidth = param.width - 2 * param.cropSizeTraining;

param.trainingScenes = 'D:\JChen\NoisyLF\_trainingViewNet\';
% param.trainingScenes = '\\eee-mtl-wdclp\jchen5\Academic\Project\_Resource\LightField\Noisy\_training\';
param.trainingData = param.trainingScenes;
% [~, param.trainingNames, ~] = GetFolderContent(param.trainingData, '.h5');

param.testData = 'D:\JChen\NoisyLF\_validationViewNet\';
% param.testData = '\\eee-mtl-wdclp\jchen5\Academic\Project\_Resource\LightField\Noisy\_validation\';
...
param.testNetIter = 100;%100;
param.printInfoIter = 5;

%%% ADAM parameters
param.alpha = 0.0001; %for rain, 16 features 
